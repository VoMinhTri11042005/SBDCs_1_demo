...
async def summary(current_user: dict = Depends(get_current_user)):
    ...
    if is_admin(current_user):
        cur.execute("SELECT COUNT(*) as c FROM appointments WHERE date(appointment_date, 'localtime') = date('now', 'localtime')")
...
async def recommendations(data: RecommendationRequest, current_user: dict = Depends(get_current_user)):
    ...
    for d in top:
        cur.execute("""
            ...
            VALUES (1, ?, ?, ?, 'NO_RESPONSE', 'NOT_SENT')
        """, (data.blood_type, d["id"], d["score"]))
        d["recommendation_result_id"] = cur.lastrowid
...
async def send_recommendation_emails(data: RecommendationEmailRequest, current_user: dict = Depends(get_current_user)):
    ...
    conn = get_db_connection(); cur = conn.cursor()
    placeholders = ','.join('?' for _ in data.recommendation_result_ids)
    cur.execute(f"""
    ...
    """, tuple(data.recommendation_result_ids))
    rows = [dict(r) for r in cur.fetchall()]
    ...
    for r in rows:
        ...
        if r.get("email_status") in {"SENT", "MOCK_SENT"}:
            ...
            cur.execute("""
                ...
                WHERE user_id=? AND blood_type=? AND status IN ('SENT','MOCK_SENT')
        ...
        if status in {"SENT", "MOCK_SENT"}:
            cur.execute("""
                ...
                SET email_status=?, email_sent_at=CURRENT_TIMESTAMP, email_error=NULL, invitation_status='SENT'
                WHERE id=?
            """, (status, result_id))
        else:
            cur.execute("""
    ...
    try:
        if is_admin(current_user):
            ...
            cur.execute("""
                ...
                WHERE date(appointment_date, 'localtime')=date('now', 'localtime') AND status IN ('PENDING','APPROVED','CHECKED_IN','IN_PROGRESS')
            """)
            today = cur.fetchone()["c"]
            if today:
            ...
            u = dict(cur.fetchone())
            cur.execute("SELECT MAX(appointment_date) AS last_completed FROM appointments WHERE donor_id=? AND status='COMPLETED'", (user_id,))
