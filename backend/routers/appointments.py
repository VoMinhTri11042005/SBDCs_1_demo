...
def _get_latest_completed(cursor, donor_id: int, exclude_appointment_id: int | None = None):
    if exclude_appointment_id:
        cursor.execute(
            ...
            WHERE donor_id = ? AND status = 'COMPLETED' AND id <> ?
...
def _build_eligibility(cursor, donor_id: int, reference_dt: datetime | None = None):
    ...
    if not last_dt:
        cursor.execute("SELECT last_donation_date FROM users WHERE id = ?", (donor_id,))
        row = cursor.fetchone()
...
async def get_my_appointments(current_user: dict = Depends(get_current_user)):
    ...
    cursor.execute("""
        ...
        FROM appointments a
        WHERE a.donor_id = ?
        ORDER BY datetime(a.appointment_date) DESC, a.created_at DESC
    """, (current_user["id"],))
...
async def cancel_my_appointment(id: int, current_user: dict = Depends(get_current_user)):
    ...
    conn = get_db_connection(); cursor = conn.cursor()
    try:
        cursor.execute("SELECT * FROM appointments WHERE id = ? AND donor_id = ?", (id, current_user["id"]))
        app = cursor.fetchone()
        if not app:
        ...
        cursor.execute("UPDATE appointments SET status='CANCELLED', notes = COALESCE(notes, '') || CASE WHEN COALESCE(notes, '') = '' THEN '' ELSE char(10) END || '[Donor cancelled]', updated_at=CURRENT_TIMESTAMP WHERE id=?", (id,))
        cursor.execute("""
            UPDATE users
