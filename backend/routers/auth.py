    ...
    try:
        ...
        if role == "HOSPITAL_ADMIN":
            cursor.execute(
                """
                UPDATE hospitals
                SET name = ?, address = ?, contact_phone = ?, contact_email = ?,
                    hospital_code = ?, city = ?, district = ?, contact_name = ?, contact_title = ?,
                    contact_person_phone = ?, contact_person_email = ?, status = 'ACTIVE'
                WHERE id = 1
                """,
                (
...
async def update_profile(data: UserUpdate, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        if user_update:
            fields = [f"{key} = ?" for key in user_update.keys()]
