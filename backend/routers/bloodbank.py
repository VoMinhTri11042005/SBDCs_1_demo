...
async def get_transactions(limit: int = 80, blood_type: str | None = None, current_user: dict = Depends(get_current_user)):
    ...
    if blood_type:
        cursor.execute("""
            ...
            WHERE hospital_id = 1 AND blood_type = ?
...
async def create_transaction(data: InventoryTransactionCreate, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        cursor.execute("""
            ...
            VALUES (?, 'CREATE_INVENTORY_TRANSACTION', 'inventory_transactions', ?, ?, ?)
        """, (current_user["id"], cursor.lastrowid, f"{data.blood_type}: {current_qty}L", f"{data.blood_type}: {new_qty}L via {tx_type} {data.quantity}L"))
        conn.commit()
